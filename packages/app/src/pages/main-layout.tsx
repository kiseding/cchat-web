  ...
  return (
    <div class="fixed inset-0 z-50 flex items-center justify-center" style="background: rgba(0,0,0,0.5)" onClick={props.onCancel}>
      <div class="rounded-2xl p-6 w-80 shadow-lg flex flex-col gap-5" style="background: var(--bg-base); border: 1px solid var(--border-base)" onClick={e => e.stopPropagation()}>
        <p class="text-xl text-center" style="color: var(--text-strong)">{props.message}</p>
        <div class="flex gap-3 justify-center">
          <button onClick={props.onCancel} class="px-5 py-2 rounded-lg text-xl cursor-pointer" style="background: var(--bg-stronger); color: var(--text-xl)">Cancel</button>
          <button onClick={props.onConfirm} class="px-5 py-2 rounded-lg text-xl font-medium cursor-pointer" style={{ background: props.confirmColor || "#dc2626", color: "white" }}>{props.confirmLabel || "Confirm"}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div class="fixed inset-0 z-50 flex items-center justify-center" style="background: rgba(0,0,0,0.5)" onClick={props.onClose}>
      <div class="rounded-2xl p-5 w-80 shadow-lg flex flex-col gap-4" style="background: var(--bg-base); border: 1px solid var(--border-base)" onClick={e => e.stopPropagation()}>
        <h3 class="text-xl font-semibold" style="color: var(--text-strong)">New Session</h3>
        <input ref={nameInputRef} type="text" value={props.name} onInput={e => { props.onInput(e.currentTarget.value) }}
          ...
          placeholder="Session name (optional)" class="px-3 py-2 rounded-lg text-xl outline-none"
          style={{ background: "var(--bg-raised)", color: "var(--text-strong)", border: `1px solid ${props.nameError ? "#dc2626" : "var(--border-base)"}` }} />
        <Show when={props.nameError}><p class="text-xl" style={{ color: "#dc2626" }}>{props.nameError}</p></Show>
        <div class="flex gap-2 justify-end">
          <button onClick={props.onClose} class="px-4 py-2 rounded-lg text-xl cursor-pointer" style={{ background: "var(--bg-stronger)", color: "var(--text-xl)" }}>Cancel</button>
          <button onClick={props.onCreate} disabled={props.creating} class="px-4 py-2 rounded-lg text-xl font-medium cursor-pointer disabled:opacity-50" style={{ background: "#34d399", color: "white" }}>Create</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      <div class="flex items-center justify-between h-12 px-3 border-b" style="border-color: var(--border-base)">
        <h2 class="font-semibold text-xl" style="color: var(--text-strong)">Sessions</h2>
      ...
      <div class="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
        <Show when={!sessions.loading} fallback={<div class="text-xl p-4 text-center" style="color: var(--text-weak)">Loading...</div>}>
          <Show when={sessions()?.length} fallback={
            <div class="text-xl p-4 text-center" style="color: var(--text-weak)">No sessions yet</div>
          }>
            <For each={sessions()}>
              {(session: SessionInfo) => (
                <div
                ...
                >
                  <div onClick={() => { navigate(`/chat/${session.id}`); props.onClose() }} class="min-w-0 flex-1 cursor-pointer">
                    <div class="text-xl truncate" style={{ color: "var(--text-strong)" }}>{session.title || "Untitled"}</div>
                    <div class="text-xl" style="color: var(--text-weak)">{session.messageCount} msgs · {formatDate(session.updatedAt)}</div>
                  </div>
                  <button onClick={() => deleteSession(session.id)} class="ml-2 px-2 py-1 rounded text-xs cursor-pointer shrink-0 font-medium" style={{ background: "#dc2626", color: "white" }}>Del</button>
                </div>
              )}
            </For>
          </Show>
...
export function MainLayout(props: { children: any }) {
  ...
  return (
    <div class="h-dvh w-screen flex flex-col overflow-hidden" style="position: relative" style="background: var(--bg-base); color: var(--text-xl)">
      {/* Overlay */}
      ...
      <header class="shrink-0 flex items-center gap-3 px-3 h-12 border-b" style="border-color: var(--border-base); background: var(--bg-raised)">
        ...
        <div class="font-medium text-xl truncate flex-1" style="color: var(--text-strong)">{sessionTitle()}</div>
